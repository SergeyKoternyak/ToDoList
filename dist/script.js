	  ...
	  function runInContext(context) {
	    ...
	    var clearTimeout = function clearTimeout(id) {
	      return context.clearTimeout.call(root, id);
	        ...
	        setTimeout = function setTimeout(func, wait) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _NotesArchive = __webpack_require__(7);

	var _NotesArchive2 = _interopRequireDefault(_NotesArchive);
	...
	var toDoApp = function () {
		function toDoApp() {
			...
			this.myNotes = new _NotesArchive2.default();
			if (localStorage.myNotes) {
				(0, _jquery2.default)(JSON.parse(localStorage.myNotes)).each(function (i, item) {
					var note = new _Note2.default(item.title, item.text, item.date, item.isDone);
					_this.myNotes.push(note);
				});
			}
		...
		_createClass(toDoApp, [{
			...
			value: function visibilityButton() {
				var _this2 = this;

				(0, _jquery2.default)('.delete-done').addClass('not-available');
				(0, _jquery2.default)(this.myNotes).each(function (i) {
					if (_this2.myNotes[i].isDone) {
						(0, _jquery2.default)('.delete-done').removeClass('not-available');
					}
				});
			}
		}, {
			...
			value: function getDate() {
				...
				var dayNow = [new Date().getDate(), new Date().getMonth() + 1, new Date().getFullYear()].join('-');
				var timeNow = [addZero(new Date().getHours()), addZero(new Date().getMinutes())].join(':');
			...
			value: function eventsInit() {
				...
				(0, _jquery2.default)('.add').click(function () {
					var note = new _Note2.default((0, _jquery2.default)('.note-title').val(), (0, _jquery2.default)('.note-body').val(), _this3.getDate());
					if (_this3.myNotes.overlapNote()) {
						_this3.myNotes.push(note);
						_this3.visibilityButton();
						(0, _jquery2.default)('.note-title').val('');
						(0, _jquery2.default)('.note-body').val('');
					}
				});
				// Действия с заметками
				(0, _jquery2.default)('.notes-list').on('click', function (e) {
					var note = (0, _jquery2.default)(e.target).closest('.note');
					var num = (0, _jquery2.default)(note).index();
					// Пометить, как выделенную / отменить
					if ((0, _jquery2.default)(e.target).hasClass('done')) {
						note.toggleClass('is-done');
						_this3.myNotes.noteIsDone(num, e.target);
						_this3.visibilityButton();
					}
					// Удалить заметку
					if ((0, _jquery2.default)(e.target).hasClass('delete')) {
						if (confirm('Подтвердите удаление')) {
							note.remove();
							_this3.myNotes.removeNote(num);
							_this3.visibilityButton();
						}
					}
					// Редактирование заметки
					if ((0, _jquery2.default)(e.target).hasClass('edit')) {
						note.find('.note-title, .note-body').attr('contenteditable', true);
						note.addClass('edit-mod');
					}
					// Сохранение редактирования
					if ((0, _jquery2.default)(e.target).hasClass('save')) {
						if (_this3.myNotes.overlapNote(note)) {
							note.removeClass('edit-mod');
							_this3.myNotes.saveEdit(num, note, _this3.getDate());
						}
					}
				});
				// Удалить все выполненные заметки
				(0, _jquery2.default)('.delete-done').click(function () {
					if (!(0, _jquery2.default)('.delete-done').hasClass('not-available') && confirm('Подтвердите удаление')) {
						_this3.myNotes.deleteDoneNotes();
						_this3.visibilityButton();
						(0, _jquery2.default)('.note.is-done').remove();
					}
				});
				// Показать поиск
				(0, _jquery2.default)('.open-search').click(function () {
					(0, _jquery2.default)('.note-create').toggleClass('show-search');
				});
				// Поиск
				(0, _jquery2.default)('.search').keyup(function () {
					_this3.myNotes.searchNotes();
				});
				// Показ/скрытие выполненных
				(0, _jquery2.default)('.hide-show').click(function () {
					(0, _jquery2.default)('.notes-list').toggleClass('hide');
					(0, _jquery2.default)('.hide-show').toggleClass('fa-eye-slash fa-eye');
				});
				// Подняться на верх страницы
				(0, _jquery2.default)('.page-up').click(function () {
					if ((0, _jquery2.default)(window).scrollTop() > 0 && (0, _jquery2.default)('body').is(':not(:animated)')) {
						(0, _jquery2.default)('body').animate({ scrollTop: 0 }, 1000);
					}
				});
				// Активация кнопки "наверх"
				(0, _jquery2.default)(window).scroll(function () {
					if ((0, _jquery2.default)(window).scrollTop() > 0) {
						(0, _jquery2.default)('.page-up').removeClass('not-available');
					} else {
						(0, _jquery2.default)('.page-up').addClass('not-available');
					}
				});
			}
		}]);
...
/***/ function(module, exports) {
	...
	var Note = function Note(title, text, date) {
		var done = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ArrayProx = function ArrayProx() {
		_classCallCheck(this, ArrayProx);
	};

	;
	ArrayProx.prototype = Array.prototype;

	var NotesArchive = function (_ArrayProx) {
		_inherits(NotesArchive, _ArrayProx);

		function NotesArchive() {
			_classCallCheck(this, NotesArchive);

			return _possibleConstructorReturn(this, Object.getPrototypeOf(NotesArchive).apply(this, arguments));
		}

		_createClass(NotesArchive, [{
			key: 'renderNote',
			value: function renderNote(note) {
				var template = (0, _jquery2.default)('#template').html();
				var compiled = _.template(template);
				var newElement = compiled(note);
				(0, _jquery2.default)('.notes-list').append(newElement);
			}
		}, {
			key: 'saveArr',
			value: function saveArr() {
				localStorage.setItem('myNotes', JSON.stringify(this));
			}
		}, {
			key: 'push',
			value: function push(note) {
				_get(Object.getPrototypeOf(NotesArchive.prototype), 'push', this).call(this, note);
				this.renderNote(note);
				this.saveArr();
			}
		}, {
			key: 'noteIsDone',
			value: function noteIsDone(num, target) {
				this[num].isDone = !this[num].isDone;
				(0, _jquery2.default)(target).attr('title', this[num].isDone ? 'Undone' : 'Done');
				this.saveArr();
			}
		}, {
			key: 'removeNote',
			value: function removeNote(num) {
				this.splice(num, 1);
				this.saveArr();
			}
		}, {
			key: 'saveEdit',
			value: function saveEdit(num, note, date) {
			...
			key: 'deleteDoneNotes',
			value: function deleteDoneNotes() {
			...
			key: 'searchNotes',
			value: function searchNotes() {
				(0, _jquery2.default)('.note').addClass('hide-el');
				if ((0, _jquery2.default)('.search').val() === '') {
					(0, _jquery2.default)('.note').removeClass('hide-el');
				}
				(0, _jquery2.default)(this).each(function (i, el) {
					if (el.title === (0, _jquery2.default)('.search').val()) {
						(0, _jquery2.default)((0, _jquery2.default)('.note')[i]).removeClass('hide-el');
					}
				});
			}
		}, {
			key: 'overlapNote',
			value: function overlapNote(note) {
				var noteTitleVal = note ? (0, _jquery2.default)(note).find('h1').text() : (0, _jquery2.default)('.note-title').val();
				var noteBodyVal = note ? (0, _jquery2.default)(note).find('p').text() : (0, _jquery2.default)('.note-body').val();
